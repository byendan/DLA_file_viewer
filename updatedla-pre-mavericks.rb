...
class LinkFinder
    ...
    def extract(url='http://wfs.sbcc.edu/Departments/LRC/computerlabonlineresources/dlas/', depth=1)
        ...
        page = self.pageLoader(url).to_ary[1]
    
        # Lambda for setting up the hash
        hash_load = lambda do |line|
            line = line.to_s.scan(/\/([^\/]*[a-z|A-Z|0-9])/)
        ...
        dir_ary.each do |d|
            ...
            content << "\t<link>#{d}</link>"
            depth_tab.call
            content << "\t<name>#{dir_hash[d].to_s.gsub(/%20/, " ")}</name>"
        ...
        file_ary.each do |f|
            ...
            content << "\t<link>#{f}</link>"
            depth_tab.call
            content << "\t<name>#{file_hash[f].to_s.gsub(/%20/, " ")}</name>"
    ...
    def writeToXml
        @content << "\n</directories>"
        File.open('linksXML.xml', 'w') do |file|
            file.write @content
        end
    end
